...
//    }
     ...
     */
    public final String userLibrary;
    @Wither
    public final String ambiguityLibrary;

    public final String crfModel;
    ...
    public final DATDictionary datDictionary;
    ...
    private AnsjContext(final ResourceBundle properties) {
        ...
        this.userLibrary = property(properties, "userLibrary", "library/default.dic");
        this.ambiguityLibrary = property(properties, "ambiguityLibrary", "library/ambiguity.dic");
        this.crfModel = property(properties, "crfModel", "library/crf.model");
        //
        ...
        this.datDictionary = new DATDictionary(this.personAttrLibrary, coreDic);
        ...
        for (final String dic : this.userLibraryResources()) {
            ...
            for (final String line : rawLines(filesystemResource(dic))) {//单个文件加载词典
                if (isNotBlank(line)) {
                    ...
                    // 如何核心辞典存在那么就放弃
                    if (this.isSkipUserDefine && this.datDictionary.getId(strs[0]) > 0) {
                        continue;
                    }
    ...
    public List<String> userLibraryResources() { // 处理词典的路径.或者目录.词典后缀必须为.dic
        final String path = this.userLibrary;
    ...
    public String ambiguityLibraryResource() {
        final String path = this.ambiguityLibrary;
    ...
    @Getter(lazy = true)
    private final UserDefineLibrary userDefineLibrary = initUserDefineLibrary();

    UserDefineLibrary initUserDefineLibrary() {
        return new UserDefineLibrary(this.loadUserDic(), this.loadUserAmbiguityDic());
    }
    ...
    SplitWord initCrfSplitWord() {
        ...
        final SplitWord crfSplitWord = new SplitWord(Model.loadModel(filesystemResource(this.crfModel)));
