...
import static org.ansj.AnsjUtils.toStream;
...
public class CrfppModelParser {
    ...
    public static Model parseCrfpp(final String path) {
        ...
        for (int i = 0; i < tmpFeatures.length; i++) { // 填充 grad
            if (i == bIndex) { // 读取转移模板
                for (int j = 0; j < tagNum; j++) {
                    for (int j2 = 0; j2 < tagNum; j2++) {
                        status[j][j2] = parseDouble(reader.readLine());
                    }
                }
                i += tagNum - 1;
                continue;
            ...
            final Feature feature = grad.get(tmpFeature.name) != null ?
                    grad.get(tmpFeature.name) :
                    new Feature(featureNum, tagNum);
            for (int j = 0; j < tagNum; j++) {
                feature.update(tmpFeature.featureId, j, parseDouble(reader.readLine()));
            }
            grad.put(tmpFeature.name, feature);
        }
    ...
    public static Template parseTemplate(final InputStream inputStream) {
        final List<String> lines = linesWithoutBlankAndComments(inputStream, line -> line);
