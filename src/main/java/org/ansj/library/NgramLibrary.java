...
import static org.ansj.AnsjItem.*;
...
public class NgramLibrary {
     ...
     * 词与词之间的关联表数据
     */
    public NgramLibrary(final List<String> bigramdict, final CoreDictionary coreDictionary) {
        ...
        bigramdict.stream().filter(StringUtils::isNotBlank).forEach(line -> {
            ...
            final AnsjItem item0 = coreDictionary.getItem(strs[0]);
            final AnsjItem item1 = coreDictionary.getItem(strs[1]);
